...
public class AuthController {
    ...
    public ResponseEntity<AuthDto.LoginResponse> socialLogin(
            @Parameter(description = "소셜 로그인 제공자 (kakao, google)", required = true , example = "kakao")
    ...
    @Operation(
            summary = "로그아웃",
            description = "현재 사용자를 로그아웃 처리합니다. 리프레시 토큰을 무효화합니다."
    )
    @PostMapping("/logout")
    ...
    public ResponseEntity<AuthDto.TokenRefreshResponse> refreshToken(
        ...
        AuthDto.TokenRefreshResponse response = AuthDto.TokenRefreshResponse.builder()
